...
1. 어떤 생성자 함수 new 연산자와 함께 호출 또는 Primitive Type 리터럴 선언하면 Constructor에서 정의된 내용을 바탕으로 새로운 인스턴스가 생성된다.
...
> 즉, Primitive Type을 리터럴로 선언하게 되면 생성자 함수를 사용하여 생성된 인스턴스로 취급한다.
# QA) 프로토타입 체인이란 무엇인가요?
> 키워드 : 메서드 오버라이딩
